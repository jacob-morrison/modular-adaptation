# gcloud alpha compute tpus tpu-vm ssh jacobm-v3-128-2 --zone=us-east1-d --project=ai2-tpu --worker=all --command="pgrep llama | xargs kill -9"

# and swap to whatever branch you want
# gcloud alpha compute tpus tpu-vm ssh jacobm-v3-128-2 --zone=us-east1-d --project=ai2-tpu --worker=all --command="git clone https://github.com/hamishivi/easylm.git"

# gcloud alpha compute tpus tpu-vm ssh jacobm-v3-128-2 --zone=us-east1-d --project=ai2-tpu --worker=all --command="cd easylm; ./scripts/tpu_vm_setup.sh"

# gcloud alpha compute tpus tpu-vm ssh jacobm-v3-128-2 --zone=us-east1-d --project=ai2-tpu --worker=all --command="python3 -m wandb login 52b8bf39aebeae0660ca724b9aac5539a6c36bf5"
...
gcloud alpha compute tpus tpu-vm ssh jacobm-v3-128-2 --zone=us-east1-d --project=ai2-tpu --worker=all --command="cd easylm; export WANDB_MODE=disabled; export LIBTPU_INIT_ARGS='--xla_jf_spmd_threshold_for_windowed_einsum_mib=0 --xla_tpu_spmd_threshold_for_allgather_cse=10000 --xla_tpu_spmd_rewrite_einsum_with_reshape=true --xla_tpu_enable_latency_hiding_scheduler=true TPU_MEGACORE=MEGACORE_DENSE'; python3 -m EasyLM.models.llama.llama_train \
    ...
    --update_llama_config='' \
    --load_dataset_state='' \
    --load_checkpoint='params::gs://jacobm-bucket/modular_adaptation/checkpoints/coding-models/llama_2_7b-tulu_all-coding_none/0ebc8901c4b4431fbc8fde2c4d1399bb/streaming_params_18788' \
    ...
    --train_dataset.json_torch_dataset.path='gs://jacobm-bucket/modular_adaptation/training_data/coding/tulu_match-coding_100.jsonl' \
    ...
    --logger.output_dir='gs://jacobm-bucket/modular_adaptation/checkpoints/coding-models/tulu_2_code_none-tulu_match-coding_100' &> all.log &"
...
# read logs on tpus:
# gcloud alpha compute tpus tpu-vm ssh jacobm-v3-128-2 --zone=us-east1-d --project=ai2-tpu --worker=all --command="cd easylm; tail -n 5 all.log"